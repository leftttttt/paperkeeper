...
if($_SERVER["REQUEST_METHOD"]== "POST"){
    ...
    if(empty($name_err) && empty($subject_err) && empty($get_date_err)){
        $sql = "INSERT INTO item (name, subject, category, get_date, due_date) VALUES (?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_subject, $param_category, $param_get_date, $param_due_date);
...
<br>
        <label for="get_date">Time You Get</label>
        <input type="datetime-local" id="get_date" name="get-date" <?php echo !empty($get_date) ? 'value="' . htmlspecialchars($get_date) . '"' : ''; ?>>
        <span style="display: block; margin-top: 10px; color: #dc3545;"><?php echo $get_date_err; ?></span>
<br>
        <label for="due_date">Due Date</label>
        <input type="datetime-local" id="due_date" name="due-date" <?php echo !empty($due_date) ? 'value="' . htmlspecialchars($due_date) . '"' : ''; ?>>
<br>
    </div>
            </div>
<br>
